...
public class AdminProductAction extends ActionSupport implements
	...
	public String save() throws IOException {
	    ...
	    if(upload != null){
			// 将商品图片上传到服务器上.
			// 获得上传图片的服务器端路径.
			String path = ServletActionContext.getServletContext().getRealPath("/images/ProductsPhoto");
			// 创建文件类型对象:
			File diskFile = new File(path+"//"+uploadFileName);
			// 文件上传:
			FileUtils.copyFile(upload, diskFile);
	
			product.setImage("images/ProductsPhoto/"+uploadFileName);
		}
	...
	public String update() throws IOException {
		// 将信息修改到数据库
		product.setPdate(new Date());
	//	imagename = UUIDUtils.getUUID();//获取字段
		// 上传:
		if(upload != null ){//默认不上传不修改 
			//将原图片删除
